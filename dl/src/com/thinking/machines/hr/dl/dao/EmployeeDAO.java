...
{
if(employeeDTO==null)throw new DAOException("Employee's null");
...
if(name.length()==0)throw new DAOException("Length of name is zero");
int designationCode=employeeDTO.getDesignationCode();
if(designationCode<=0)throw new DAOException("Invalid designationCode :"+designationCode);
DesignationDAOInterface designationDAO;
designationDAO=new DesignationDAO();
boolean isDesignationCodeValid=designationDAO.codeExists(designationCode);
if(isDesignationCodeValid==false)throw new DAOException("Invalid desination code: "+designationCode);
Date dateOfBirth=employeeDTO.getDateOfBirth();
if(dateOfBirth==null)throw new DAOException("Date of birth is null");
char gender=employeeDTO.getGender();
...
if(panNumber==null)throw new DAOException("Pan number is null");
panNumber=panNumber.trim();
if(panNumber.length()==0)throw new DAOException("Lengthof pan number is zero");
String aadharCardNumber=employeeDTO.getAadharCardNumber();
...
{
throw new DAOException("Invalid designaton code : "+designationCode);
}
Set<EmployeeDTOInterface> employees=new TreeSet<>();
try
{
